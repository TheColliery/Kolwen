...
- **Every published claim must match what the code does.** The rule is universal; the machine
  ...
  surface; an internal path, a private-repo reference or an absolute local path; the owner's private
  login address, in any of its spellings, on a tracked text file; Thai orthography
