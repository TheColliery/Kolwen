...
| A page's inline script stops running, or the CSP drifts from what the page carries | the CSP hashes are recomputed from the HTML on every push, and the served headers, the 404 page included on `kolwen.com` when a hand run can reach it (CI cannot), are compared to `web/_headers` after deploy | `scripts/surface-check.mjs`, `scripts/post-deploy-check.mjs` |
