...
**เครื่องหมายเป็นรูปนามธรรม เหมือนกับชื่อ — อธิบายด้วยเรขาคณิตเท่านั้น** ห้ามเรียกด้วยคำที่เป็นวัตถุ
(บันได · ขั้นบันได · ชั้น · ไม้บรรทัด ฯลฯ) ทั้งในเอกสาร ใน alt text และในบรีฟงานออกแบบ:
แถบสามแถบ **กว้างเท่ากันทั้งสามแถบ (64.3)** · ความสูงลดลงทีละ **×0.786** ต่อแถบ ·
ขอบซ้ายเลื่อนไปทางขวาทีละ **+9.35** คงที่ · แถบบนสุดมีอัตราส่วน ยาว:สูง = **2.572**
(ตัวเลขทั้งหมดอ่านจาก `make-brand.mjs` ซึ่งเป็นต้นทางเดียวของรูปทรง — ถ้าเอกสารกับสคริปต์ไม่ตรงกัน
สคริปต์ถูก) **ไม่มี design pass ใดผลักเครื่องหมายไปทางคำเปรียบเทียบได้** รูปนามธรรมไม่ต้องแปลว่าอะไร
...
**สามไฟล์ใน `web/` ถูก `make-brand.mjs` เขียนให้ ไม่ใช่ก๊อปมา** — `web/favicon.svg` มาจากการเรียก
ฟังก์ชันเดียวกับ `kolwen-icon.svg` ส่วน `web/favicon-32.png` และ `web/apple-touch-icon.png` เป็น buffer
ตัวเดียวกับ `kolwen-favicon-32.png` และ `kolwen-icon-256.png` **ห้ามแก้ไฟล์ฝั่ง `web/` ด้วยมือ** แก้ที่
สคริปต์แล้ว generate ใหม่ CI เทียบไบต์ทั้งสองฝั่ง
...
## 9. ชั้นถ่านหิน (rank) — ชื่อรุ่น

**สี่ชั้นที่ Kolwen ใช้ เรียงตามลำดับการเกิดถ่านหิน:** **Peat · Lignite · Bituminous · Anthracite**

ชั้นที่อยู่ระหว่างกลาง — sub-bituminous, semi-anthracite, meta-anthracite — **ไม่ใช่ tier แต่เป็น
"รอยต่อ" (seam)** คือชั้นเปลี่ยนผ่านที่มาตรฐานถ่านหินเก็บไว้สำหรับกรณีคาบเกี่ยว **ใช้ได้เฉพาะเมื่อค่าที่วัด
ได้ของรุ่นนั้นตกอยู่ระหว่างสองชั้น** แล้วเรียกด้วยชื่อรอยต่อนั้น **ห้ามปัดขึ้นไปชั้นบน**

**Bituminous = ชั้นที่มีชื่อ แต่ยังไม่มีรุ่นจริง → สงวนไว้** ปรากฏเป็นสินค้าได้ก็ต่อเมื่อมีรุ่นที่วัดได้ถึง
ปัจจุบันมีรุ่นที่ประกาศแล้วสามชั้น (Peat · Lignite · Anthracite) ไม่นับ Kolwen ตัวฟรี

### กฎการจับคู่ — บรรทัดที่รับน้ำหนักทั้งหมด

**ชั้นได้มาด้วยการวัด ไม่ใช่ด้วยการตั้งชื่อ** ถ่านหินจริงจัดชั้นด้วยตัวเลข (fixed carbon / ค่าความร้อน)
ไม่ใช่ด้วยความรู้สึกของคนขาย — ของเราก็เหมือนกัน: **ต้องประกาศตัววัดและค่าที่ได้ก่อน แล้วชื่อชั้นจึงตามมา
ห้ามใช้ชื่อชั้นเป็นคำโฆษณา** รุ่นที่ยังไม่มีค่าที่ประกาศ = ยังไม่มีชั้น

### ที่มาของลำดับ — อ้างมาตรฐาน ไม่ได้คิดเอง

ลำดับชั้นยืมมาจากมาตรฐานการจัดชั้นถ่านหิน **ASTM D388** (US) และ **ISO 11760** (สากล)

⚠️ **เกณฑ์ตัวเลขของมาตรฐานทั้งสองฉบับ ยังไม่ได้ยืนยันจากต้นฉบับ** — astm.org ตอบ 403 และหน้า
catalogue ของ iso.org เป็น JavaScript ล้วน ดึงข้อความไม่ออก (ลอง 2026-09-03) **จึงไม่ยกตัวเลข
clause ของมาตรฐานมาอ้าง** และห้ามใครเติมจากความจำ

**ค่าที่ยืนยันได้จริง (U.S. Energy Information Administration, ดึงสด 2026-09-03)** — เป็นแหล่งรัฐบาล
ปฐมภูมิ ไม่ใช่ตัวมาตรฐาน ระบุที่มาไว้อย่างนั้น: lignite 25–35% carbon · sub-bituminous 35–45% ·
bituminous 45–86% · anthracite 86–97% และระบุว่า **ชั้นถูกกำหนดด้วยความดันและความร้อนที่กระทำ
ต่อซากพืชตามเวลา**

### เรื่องเล่าเบื้องหลัง

ถ่านหินคือประวัติศาสตร์โลกที่ถูกบีบอัด — ซากพืชนับล้านปีถูกความร้อนและความดันบีบจนกลายเป็นก้อนที่
ให้พลังงานต่อหน่วยสูงขึ้นเรื่อย ๆ **ชั้นที่สูงกว่า = ถูกบีบอัดมากกว่า = พลังงานต่อหน่วยมากกว่า**

รุ่นของเราเกิดแบบเดียวกัน: เวลาและข้อมูลจำนวนมหาศาลถูกบีบลงเป็นน้ำหนักโมเดลก้อนเดียว
**ชื่อชุด (TheColliery) บอกว่าใครเป็นคนผลิต · ชื่อชั้น (rank) บอกว่าเชื้อเพลิงก้อนนั้นถูกบีบมาแค่ไหน**

**เรื่องเล่านี้ไม่แตะเครื่องหมาย** ข้อ 6 ยังบังคับเต็มที่ — เครื่องหมายเป็นรูปนามธรรม อธิบายด้วยเรขาคณิต
เท่านั้น ชั้นถ่านหินตั้งชื่อ **เชื้อเพลิง** ไม่ได้ตั้งชื่อ **รูป** และห้ามใช้เรื่องเล่านี้ไปอธิบายสามแถบ
### ชื่อรุ่นต้องมี "Kolwen" นำหน้าเสมอ — เป็นกฎ ไม่ใช่คำแนะนำ

**เขียนว่า "Kolwen Anthracite" เท่านั้น ห้ามใช้คำว่า "Anthracite" ลอย ๆ** — และเช่นเดียวกันกับทั้งสี่ชั้น
ชื่อชั้นถ่านหินเป็นคำสามัญ ใครก็ใช้ได้ สิ่งที่แยกสินค้าเราออกจากของคนอื่นคือคำว่า **Kolwen** ไม่ใช่ชื่อชั้น
เจ้าของตัดสินแล้วว่า **เดินหน้าทั้งสี่ชื่อ** พร้อมกฎข้อนี้ ไม่ใช่เปลี่ยนชื่อ

**ผลตามมาที่ต้องทำจริง:** ชื่อไฟล์ · ชื่อ repo · ชื่อโมเดลบน Hugging Face · หัวข้อในเอกสาร · ป้ายในหน้าเว็บ
ทุกที่ต้องขึ้นต้นด้วย `Kolwen` การเรียกสั้น ๆ ว่า "Anthracite" ในบทสนทนาไม่เป็นไร แต่**ในข้อความที่เผยแพร่
ห้าม**

### รายชื่อที่เฝ้าดู (watch list)

อ่านจาก TMview (EUIPN, ข้อมูล "updated 05/09/2026") ซึ่งรวม EUIPO + USPTO + WIPO + UKIPO + DPMA +
INPI + กรมทรัพย์สินทางปัญญาไทย และสำนักงานอื่นอีกราว 70 แห่ง **TMview ไม่ใช่ทะเบียนทางการ ไม่มีผลทาง
กฎหมาย** — ถ้าจะดำเนินการทางกฎหมายจริงต้องกลับไปตรวจที่สำนักงานต้นทางอีกครั้ง

| เครื่องหมาย | ทะเบียน | เลขที่ | ทำไมต้องเฝ้า | วันที่อ่าน |
|---|---|---|---|---|
| `ANTHROCITE` | USPTO | 99919064 | สะกดใกล้กันมาก · class 42 · ยื่น 02/07/2026 · **ยังไม่จบกระบวนการ** | 2026-09-05 |
| `anthracite-org` | — (ไม่ใช่เครื่องหมายการค้า) | — | กลุ่ม open-weights LLM ที่ยังเคลื่อนไหว ปล่อยตระกูล **Magnum** บน Hugging Face — **สัญญาณเรื่องความสับสนและการค้นเจอ ไม่ใช่เรื่องกฎหมาย** | 2026-09-05 |
| `Anthracite` | INPI (FR) | 5157492 | มีผลอยู่ · class 9/16/41 · ยื่น 19/06/2025 | 2026-09-05 |
| `ANTHRACITE` | DPMA (DE) | — | มีผลอยู่ · class 9/38/41 · ยื่น 09/03/2020 | 2026-09-05 |
| `PEAT` | USPTO | 87464103 | มีผลอยู่ · class 9/41 · ซอฟต์แวร์ข้อสอบ (2017) | 2026-09-05 |
| `PEAT` | UKIPO | — | มีผลอยู่ · class 16/35/41/42 (2011) | 2026-09-05 |
| `KALIGNITE` | UKIPO · EUIPO · WIPO · USPTO · JPO | — | จดแล้วทุกที่ · ซอฟต์แวร์ ATM/การเงิน class 9/42 · **เป็นคนละคำ ไม่ใช่ "Lignite"** ความเสี่ยงสับสนต่ำ บันทึกไว้เฉย ๆ | 2026-09-05 |

บางช่องเว้น **—** ไว้โดยตั้งใจ: เลขทะเบียนบางรายการยาวถึงเกณฑ์ที่ `scripts/surface-check.mjs` ถือว่าเป็น
**ตัวระบุ** และห้ามปรากฏบนหน้าสาธารณะ กฎนั้นเขียนเป็นรูปแบบ ไม่ใช่รายชื่อ จึงไม่ยกเว้นให้ทีละกรณี —
เลขเต็มอยู่ในบันทึกฝั่งภายใน ผู้อ่านทั่วไปไม่ต้องใช้เลขเพื่อค้นเจอเครื่องหมายเหล่านี้บน TMview

**สิ่งที่การอ่านครั้งนี้ตอบชัด:** ไม่มีเครื่องหมายคำว่า **"Lignite"** ใน class 9 หรือ 42 เลย · **"Bituminous"**
ว่างสำหรับซอฟต์แวร์/AI · **"Peat"** แน่นที่สุดในสี่คำ (คำสามัญ + เป็นนามสกุลในชื่อ KPMG) · **"Anthracite"**
คือกรณีที่ต้องเฝ้า

**ขอบเขตที่ยังไม่ได้อ่าน บอกไว้ไม่ให้เข้าใจว่าครบ:** class 42 ของคำว่า Peat อ่านแบบ `Contains` หน้าแรก
จาก 323 หน้า (โหมด whole-word รันซ้ำไม่สำเร็จ) ส่วน class 9 อ่านครบ 85 แถว
